...
public class UserSearchHandler implements OnRequestCallback {
    ...
    private final ServiceRegistryAndDiscovery serviceRegistry;

    public UserSearchHandler(ServiceRegistryAndDiscovery serviceRegistry) {
    ...
    private FrontendSearchResponse createFrontendResponse(FrontendSearchRequest frontendSearchRequest) {
        SearchModel.Response searchClusterResponse =
                sendRequestToSearchCluster(frontendSearchRequest.getSearchQuery());
    ...
    private List<FrontendSearchResponse.SearchResultInfo> filterResults(SearchModel.Response searchClusterResponse,
        ...
        for (int i=0; i<searchClusterResponse.getRelevantDocumentsCount() && i < maxResults; i++) {
            int normalizedScore = normalizeScore(searchClusterResponse.getRelevantDocuments(i).getScore(), maxScore);
            ...
            FrontendSearchResponse.SearchResultInfo resultInfo
                    = new FrontendSearchResponse.SearchResultInfo(title, extension, normalizedScore);
    ...
    private SearchModel.Response sendRequestToSearchCluster(String searchQuery) {
        ...
        try {
            String coordinatorAddress = serviceRegistry.getRandomServiceAddress();
